...
class condGANTrainer(object):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                if gen_iterations % 1000 == 0:
                    backup_para = copy_G_params(netG)
                    load_params(netG, avg_param_G)
                    self.save_img_results(netG, fixed_noise, sent_emb_zsl_gan,
    ...
    def gen_example(self, captions, tfidfs, cap_lens, sorted_indices):
        if cfg.TRAIN.NET_G == '':
            print('Error: the path for morels is not found!')
        else:
            # Build and load the generator
            ...
            save_dir = '%s/%s' % (s_tmp, 'manual_test')
            mkdir_p(save_dir)
            ...
            tfidfs = Variable(torch.from_numpy(tfidfs), volatile=True)
            ...
            for i in range(1):  # 16
                ...
                ######################################################
                noise.data.normal_(0, 1)
                fake_imgs, attention_maps, _, _ = netG(noise, sent_emb_zsl_gan, words_embs, mask)
                # G attention
